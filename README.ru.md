...
> ✅ **v1.1.0: критичный баг с перезагрузкой исправлен.** До этой версии туннели не переживали перезагрузку системы. Подробности — в разделе [История критичных находок](#история-критичных-находок). 
...
- [История критичных находок](#история-критичных-находок)
...
По умолчанию `interface_configure()` pfSense устанавливает MTU интерфейса AWGCLIENT в **1500**, если соответствующее поле не заполнено. Для корректной работы протокола AmneziaWG (с учетом накладных расходов на инкапсуляцию и обфускацию трафика) оптимальным значением MTU является **1376**. При возникновении проблем с фрагментацией пакетов или «зависанием» соединений, принудительно задайте значение `1376` в поле **MTU** на странице **Interfaces → AWGCLIENT** (обратите внимание: настройка производится в системном интерфейсе, а не в форме плагина — см. Roadmap).
...
```
## История критичных находок

Этот проект дался нелегко — несколько архитектурных препятствий пришлось обходить через практическое тестирование, а не документацию (которая для многих из этих случаев просто не существует). Кратко, для тех, кто пойдёт похожим путём:

1. **Исходники ядра pfSense для веток 2.8.x не публикуются Netgate.** Из-за этого сборка кернел-модуля AmneziaWG невозможна — отсюда архитектура полностью на userspace (`amneziawg-go`).
2. **`write_config()` не сериализует глубоко вложенные структуры произвольных пакетов** в этой сборке pfSense — ни через `config_set_path()`, ни через прямое присвоение `$GLOBALS['config']`. Решение — собственный JSON-файл для хранения туннелей.
3. **Двойное управление адресом интерфейса ломает NAT/gateway.** Назначение адреса напрямую через `ifconfig` в обход pfSense конфликтовало со Static IPv4 в Interfaces GUI. Решение — только `interface_configure()`.
4. **Имена интерфейсов `awgN` не распознавались как виртуальные** функцией `is_interface_mismatch()` (регэксп `^tun|^wg|^ovpn|...` в `/etc/inc/util.inc`) — pfSense считал их физически отсутствующими при загрузке. Решение — переименование в `tun9NNN`.
5. **Флаг `-f` в `daemon(8)` означает не «форк», а «перенаправить вывод дочернего процесса в `/dev/null`»** (см. `man daemon`) — мы годами теряли реальные логи и ошибки демона из-за неверного предположения о семантике флага.
6. **`custom_php_resync_config_command` не вызывается автоматически при обычной загрузке** — этот хук предназначен для других сценариев (пересинхрон из Package Manager).
7. **Поздний внутренний проход pfSense по интерфейсам уничтожал туннели, поднятые через `rc.d`**, считая их не полностью настроенными штатным образом. Решение — перенос запуска в нативный `<shellcmd>`, исполняемый в самом конце загрузки.
8. **`<shellcmd>` должен лежать по пути `system/shellcmd` в `config.xml`, а не в корне.** Функция `system_do_shell_commands()` в `system.inc` читает `config_get_path("system/shellcmd", '')` — запись в неверном месте XML-дерева молча игнорируется без единой ошибки.
...
- [x] ~~**Программное применение MTU** — сейчас MTU нужно задавать вручную на странице интерфейса pfSense (см. выше); в планах — автоматическое применение рекомендованного значения `1420` после `interface_configure()`.~~ - — решено не автоматизировать частично то, что и так требует ручной настройки интерфейса в GUI pfSense.
