...
- [ ] **Чекбокс включения/выключения отладочного логирования** на странице **VPN → AmneziaWG → Туннели** — сейчас `AWG_DEBUG = true` жёстко закодирован в `awg.inc`, что означает постоянную запись отладочных сообщений в системный лог.
