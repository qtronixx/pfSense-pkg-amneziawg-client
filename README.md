...
- [x] ~~**Programmatic MTU application** — MTU currently has to be set manually on the pfSense interface page (see above); the plan is to automatically apply the recommended `1420` value after `interface_configure()`.~~ — it was decided not to partially automate tasks that already require manual interface configuration within the pfSense GUI.
